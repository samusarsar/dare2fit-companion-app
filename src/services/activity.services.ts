...
export const logActivity = ({
  ...
  return get(ref(db, `logs/${handle}/${todayDate}/${activityType}`)).then(
    (snapshot) => {
      if (!snapshot.exists()) {
        return update(ref(db, `logs/${handle}/${todayDate}`), {
          [activityType]: loggedValue,
        });
      }
...
export const getUserLogByDate = (handle: string, date: string) => {
  return get(ref(db, `logs/${handle}/${date}`)).then((snapshot) => {
    if (!snapshot.exists()) {
 ...
 */
export const getAllLogs = (): Promise<{
  [key: string]: { [key: string]: ITodayLog };
}> => {
  return get(ref(db, "logs")).then((snapshot) => {
    if (!snapshot.exists()) {
      throw new Error("No log for this date");
    }
