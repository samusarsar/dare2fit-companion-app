...
export const addWorkout = (workout: IWorkoutFormValues, handle: string) => {
  ...
  return push(ref(db, "workouts"), {
    ...workout,
    createdOn: moment(new Date()).format("DD/MM/YYYY HH:mm:ss"),
  }).then((result) => {
    const workoutId = result.key;
    update(ref(db, `workouts/${workoutId}`), { workoutId });
...
export const addWorkoutToUser = (workoutId: string, handle: string) => {
  return get(ref(db, `users/${handle}/workouts`)).then((snapshot) => {
    if (snapshot.exists()) {
...
export const getWorkoutById = (workoutId: string) => {
  return get(ref(db, `workouts/${workoutId}`)).then((snapshot) => {
    if (!snapshot.exists()) {
  ...
  return get(ref(db, `users/${handle}/workouts`))
    ...
    .then((data) => {
      const promises = Object.keys(data).map((workoutId) => {
        return getWorkoutById(workoutId).catch(() => {
...
export const sortWorkoutsByDate = (workouts: IWorkout[] | []) => {
  return [...workouts].sort((a, b) =>
    moment(b.createdOn, "DD/MM/YYYY HH;mm;ss").diff(
      moment(a.createdOn, "DD/MM/YYYY HH;mm;ss")
    )
  );
};
...
export const deleteWorkout = (workoutId: string, handle: string) => {
  return remove(ref(db, `workouts/${workoutId}`)).then(() =>
    removeWorkoutFromUser(workoutId, handle)
  );
};
